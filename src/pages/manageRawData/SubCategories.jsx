...
import useStore from '../../store/useSubCateStore';
...
export default function SubCategories() {
  ...
  const { subcategorys, isLoading, error, fetchSubcategory, addSubcategory, editSubcategory, removeSubcategory } = useStore();
  ...
  useEffect(() => {
    const filtered = subcategorys.filter(item =>
  ...
  const handleData = (setData) => {
    ...
    if (setData) {
      setValue('category', setData.category);
    } else {
      reset(); 
    }
  };
  ...
  const currentData = (filteredData.length ? filteredData : subcategorys).slice(
    indexOfFirstItem,
    indexOfLastItem
  );

  const totalPages = Math.ceil(
    (filteredData.length ? filteredData : subcategorys).length / itemsPerPage
  );
  ...
  const onSubmitModal = async (data) => {
    ...
    if (res.isConfirmed) {
      try {
        if (modalData) {
          await editSubcategory({ id: modalData.id, ...data });
        ...
        fetchSubcategory(); // ตรวจสอบว่าฟังก์ชันนี้ทำงานถูกต้อง
        reset(); 
        setModalData(null);
      } catch (error) {
    ...
    <Layout>
      <div className="container-fluid h-100 p-5">
        <div className="d-flex justify-content-between">
          <div className="h3">จัดการหมวดหมู่ย่อย</div>
          <div className="">
            <button
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Table tbSty="table table-white table-hover mt-4">
              ...
              <tfoot>
                <tr>
                  <th className="text-center">จำนวนแผนทั้งหมด {filteredData.length || subcategorys.length}</th>
          ...
          <div className="row">
            <div className="col-md-5">
              <Inputs
                ...
                name="category"
                type="text"
                placeholder={modalData ? "แก้ไขหมวดหมู่ย่อย" : "เพิ่มหมวดหมู่ย่อย"}
                register={modalRegister}
                rules={{ required: "ระบุหมวดหมู่ย่อย" }}
                errors={modalErrors}
              />
